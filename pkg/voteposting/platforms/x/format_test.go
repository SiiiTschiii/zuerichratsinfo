...
func TestFormatVoteThread_PreservesPostulatMotion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			thread := FormatVoteThread(tt.votes, nil)
...
func TestFormatVoteThread_RootTruncation(t *testing.T) {
	// Create a vote with a very long title that exceeds maxChars
	longTitle := strings.Repeat("A", maxChars+500)
	votes := []zurichapi.Abstimmung{
		{
	...
	root := thread[0].Text
	if len(root) > maxChars {
		t.Errorf("root post exceeds maxChars: %d > %d", len(root), maxChars)
	}
...
func TestFormatVoteThread_EmptyVotes(t *testing.T) {
	thread := FormatVoteThread(nil, nil)
	if thread != nil {
